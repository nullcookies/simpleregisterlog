 ...
 */
abstract class AbstractGraphTable extends nomvcAbstractTable {

	protected $tableTemplate = 'component/graph';
	...
	protected function runAsHtml() {
		...
		$series = array(array('name' => 'Токены', 'data' => array()));
		$names = array();
		foreach ($data as $row) {
			$series[0]['data'][] = intval($row->cnt);
			$names[] = $row->dt;
		}
